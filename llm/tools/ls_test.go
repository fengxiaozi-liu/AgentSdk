...
func TestPrintTree(t *testing.T) {
	...
	dirSuffix := string(filepath.Separator)

	// Check the output format
	assert.Contains(t, result, "- /root"+dirSuffix)
	assert.Contains(t, result, "  - dir1"+dirSuffix)
	assert.Contains(t, result, "    - file1.txt")
	assert.Contains(t, result, "    - subdir"+dirSuffix)
