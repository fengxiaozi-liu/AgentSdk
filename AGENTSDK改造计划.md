...
后续 CLI 项目可以引入本 SDK，并在 CLI 层创建 LSP 相关工具、补全、主题、spinner 等交互能力。
...
| 问题 | 当前表现 | 影响 |
|------|----------|------|
| SDK 和 CLI 边界不清 | `infra/format`、`infra/theme`、`extensions/completions` 留在 SDK 内 | SDK 携带终端展示和交互假设 |
| 基础工具和 LSP 强绑定 | `view/edit/write/patch/diagnostics` 直接依赖 LSP client | 不启用 LSP 时工具层仍被扩展污染 |
| tools 分层不完整 | 有 `base/mcp`，但 Skill 还没有成为平级工具来源 | 后续 Skill 接入缺少清晰位置 |
...
这样 SDK 核心不依赖 LSP，但 CLI 可以按需注入 LSP diagnostics hook。

### 4.4 MCP 与 Skill 平级

`MCP` 和 `Skill` 都属于 `tools` 下的工具来源。
...
| tools 结构 | `base/mcp/agent`，Skill 未成型 | `core/base/mcp/skill` |
| LSP | base tools 直接依赖 LSP | SDK 只保留 `extensions/lsp`，工具由 CLI 创建 |
...
| prompt | 较厚的系统提示词构造 | 薄 prompt，可由调用方传入 |
...
- 删除 `infra/format`
- 删除或迁出 `infra/theme`
- 删除或迁出 `extensions/completions`

注意：

- 删除 `theme` 前先确认 `utils/diff` 不再依赖终端颜色。
- 补全能力由未来 CLI 项目实现。
...
- 新增 Skill loader。
- 新增 Skill registry。
- 新增 Skill Tool adapter。
- 与 `tools/mcp` 保持平级。

验收：

- Agent 可同时加载 base tools、mcp tools、skill tools。
- Skill 正文按需加载，不在启动时全量注入上下文。

### Phase 6: Data/Repo 改造
...
- 核心运行时清晰。
- `tools/core/base/mcp/skill` 边界稳定。
- LSP 不进入 SDK 默认 tools。
- CLI/TUI 能力全部迁出。
- 文件工具通过 hook 扩展后续能力。
- 数据层具备 repo 抽象。
