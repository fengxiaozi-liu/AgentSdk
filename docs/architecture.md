...
| Providers | `internal/llm/provider` | 适配 OpenAI-compatible、Anthropic、Gemini、Bedrock、Azure、Copilot、VertexAI、OpenRouter、GROQ、XAI、本地和 mock Provider。 |
| 模型元数据 | `internal/llm/models` | 定义 `Model`、Provider/Model ID，并从 `models.json` 加载按 Provider 划分的模型示例。 |
| Prompt 解析器 | `internal/service/prompt` | 加载 JSON/YAML prompt 配置，并按 key 解析 system prompt。默认 prompt 位于 `internal/service/prompt/prompts.json`。 |
...
| Services | `internal/service/session`、`internal/service/message`、`internal/service/history` | 基于 repo 契约暴露领域服务，并发布 pubsub 事件。 |
| 工具协议 | `internal/tools` | 定义 `BaseTool`、工具调用/响应、文件事件、文件 hooks，以及 hook 结果合并逻辑。 |
| 基础工具 | `internal/tools/workspace` | 提供 SDK 安全的工具，例如文件 view/edit/write/patch、grep/glob/ls、bash、fetch 和 sourcegraph。 |
| MCP 工具 | `internal/mcp` | 发现并执行来自已配置 MCP server 的工具。 |
...
- 硬编码 prompt 源文件，例如 `internal/service/prompt/coder.go`、`internal/service/prompt/title.go`、`internal/service/prompt/task.go` 和 `internal/service/prompt/summarizer.go`。
...
graph TD
    ...
    Agent --> Sessions[internal/service/session.Service]
    Agent --> Messages[internal/service/message.Service]
    Agent --> Provider[internal/llm/provider.Provider]
    Agent --> Tools[internal/tools.BaseTool set]
    Sessions --> Repos[internal/data/repo]
    Messages --> Repos
    History[internal/service/history.Service] --> Repos
    Repos --> Data[internal/data/db gorm]
    Tools --> Base[internal/tools/workspace]
...
`internal/service/prompt.ResolveSystemPromptByKey(key)` 支持 JSON 和 YAML。如果设置了 `internal/config.PromptConfigPath`，则使用该文件；否则使用内嵌的 `internal/service/prompt/prompts.json` 默认配置。
