...
func NewDiagnosticsTool(lspClients map[string]*lsp.Client) toolcore.BaseTool {
	return &diagnosticsTool{
		lspClients,
	}
}

func (b *diagnosticsTool) Info() toolcore.ToolInfo {
...
func (b *diagnosticsTool) Run(ctx context.Context, call toolcore.ToolCall) (toolcore.ToolResponse, error) {
	...
	if len(lsps) == 0 {
		return toolcore.NewTextErrorResponse("no LSP clients available"), nil
	}
